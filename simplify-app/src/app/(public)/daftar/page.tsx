...
function DaftarContent() {
  ...
  const handleGoogleLogin = () => {
    ...
    try {
      account.createOAuth2Token(
        ...
        `${window.location.origin}/daftar?error=oauth_failed`
      );
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(e.message || 'Gagal mendaftar. Email mungkin sudah terpakai.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4 py-12 bg-gray-50/50">
      <div className="max-w-md w-full">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Bergabung Sekarang</h1>
          <p className="text-sm text-gray-500 mt-2">
            Buat akun gratis untuk menyimpan buku favorit Anda.
          </p>
        </div>
        ...
        <div className="card p-6 sm:p-8">
          <form onSubmit={handleSubmit} className="space-y-5">
            {(error || oauthError) && (
              <div className="bg-red-50 border border-red-200 text-red-600 text-sm p-3 rounded-lg">
                {error || (oauthError === 'oauth_failed' ? 'Gagal mendaftar dengan Google.' : oauthError)}
              </div>
            )}
            ...
            <div className="relative flex items-center py-2">
              ...
              <span className="flex-shrink-0 mx-4 text-gray-400 text-xs">ATAU DAFTAR MANUAL</span>
              <div className="flex-grow border-t border-gray-200"></div>
            </div>
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            Sudah punya akun?{' '}
            <Link href="/masuk" className="text-primary-600 font-semibold hover:underline">
              Masuk di sini
            </Link>
          </p>
        </div>
